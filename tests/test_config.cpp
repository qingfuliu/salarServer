...
sylar::ConfigVar<std::map<std::string,int> >::ptr p8=sylar::Config::Lookup("system.map",
                                                       std::map<std::string,int>{{"port",2},{"k",2},{"p",2}}
                                                         ,"system.map");
sylar::ConfigVar<std::set<LogDefiner>>::ptr Log_Config1=sylar::Config::Lookup("logs",
                                                                            std::set<LogDefiner>{},
                                                                             "log");
...
void test_yaml_logers(){
        YAML::Node root=YAML::LoadFile("/home/lqf/cpp/sylar/conf/config.YAML");
        Config::LoadFromYaml(root);
        for(auto &val:Log_Config1->getVal()){
            std::cout<<val.name<<std::endl;
            std::cout<<val.formatter_<<std::endl;
            for(auto &p:val.appenders_){
                std::cout<<p.file<<std::endl;
                std::cout<<p.type<<std::endl;
                std::cout<<p.formattor<<std::endl;
            }
        }
        std::cout<<Log_Config1.use_count()<<std::endl;
        SYLAR_LOG_DEBUG(LOGGER_DEBUG_ROOT)<< p1->toString();
       // std::cout<<"val.formatter_"<<std::endl;
       // std::stringstream ss;
       // ss<<root;
